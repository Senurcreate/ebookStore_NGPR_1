...
const AudiobookPage = () => {
  ...
  const formatBookData = (book) => ({
    ...
    // Audio specific fields can be passed through spread (...book)
  });

  // Load Dynamic Filter Options (Runs once on mount)
  useEffect(() => {
    const loadFilters = async () => {
      // Fetches available authors/genres/languages from backend
      const data = await fetchFilterOptions();
      
      if (data) {
        setAuthorOptions(["Authors", ...data.authors]);
        setGenreOptions(["Genres", ...data.genres]);
        setLanguageOptions(["Languages", ...data.languages]);
      }
    };
    loadFilters();
  }, []);

  // Fetch Audiobooks (Runs when filters or page changes)
  useEffect(() => {
    const loadAudiobooks = async () => {
      ...
      try {
        ...
        if (filters.language !== "Languages") params.language = filters.language;
        ...
        setError("Something went wrong while fetching audiobooks.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="audiobook-page">
      <div className="container">
        ...
        {!loading && !error && (
            ...
            Showing {books.length} audiobooks
            </p>
        )}
