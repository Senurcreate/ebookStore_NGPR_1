...
const StarRating = ({ rating, editable = false, onRatingChange, hoverRating = 0, onHoverChange, showNumber = true }) => {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    stars.push(
      <i
        key={i}
        className={`bi ${starClass} star ${editable ? 'editable' : ''}`}
        style={{ cursor: editable ? 'pointer' : 'default' }}
        onMouseMove={(e) => {
          if (editable && onHoverChange) {
            const val = getRatingFromEvent(e, i);
            onHoverChange(val);
          }
        }}
        onMouseLeave={() => {
          if (editable && onHoverChange) onHoverChange(0);
        }}
        onClick={(e) => {
  ...
  return (
    <div className="star-rating-container d-flex align-items-center">
      ...
      {showNumber && rating > 0 && <span className="rating-number ms-2">{rating.toFixed(1)}</span>}
    </div>
  );
};
...
const CommentSection = ({ bookId, onReviewAdded }) => {
  ...
  // Safe User Object Construction with PHOTO
  const loggedInUser = mongoUser ? {
    id: mongoUser._id, 
    name: mongoUser.displayName || 'User',
    photo: mongoUser.photoURL || currentUser?.photoURL || DEFAULT_PROFILE_PIC, // Added Photo
    isAdmin: mongoUser.role === 'admin' || mongoUser.role === 'moderator'
  } : (currentUser ? {
      id: currentUser.uid, 
      name: currentUser.displayName,
      photo: currentUser.photoURL || DEFAULT_PROFILE_PIC, // Added Photo
      isAdmin: false
  } : null);
  ...
  const handlePostReview = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        // Optimistically add photo to the new review object so it shows immediately
  ...
  return (
    <div className="container mt-5 mb-5 pt-5 pb-5 comment-section">
      ...
      <style>{`
        ...
        .review-header .d-flex, .reply-card .d-flex {
            align-items: center;
        }
      `}</style>
      ...
      {flagPopup.show && (
        <div 
          ...
          style={{ top: `${flagPopup.position.top}px`, left: `${flagPopup.position.left}px` }}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="popup-header">
            <i className="bi bi-flag-fill text-danger me-2"></i>
            <span>Report {flagPopup.type}</span>
            <button type="button" className="popup-close" onClick={cancelFlag}><i className="bi bi-x"></i></button>
          </div>
          <div className="popup-body">
            <p className="mb-2">Report this {flagPopup.type} by <strong>{flagPopup.userName}</strong>?</p>
            <p className="text-muted small mb-0"><i className="bi bi-info-circle me-1"></i> Moderators will review this content.</p>
          </div>
          <div className="popup-footer">
            <button type="button" className="btn btn-outline-secondary" onClick={cancelFlag}>Cancel</button>
            <button type="button" className="btn btn-sm btn-danger" onClick={confirmFlag}>
      ...
      <div className={`alert ${loggedInUser ? 'alert-info' : 'alert-warning'} d-flex align-items-center mb-4`}>
        {loggedInUser ? (
            <>
                ...
                />
                <div>
                    <strong>Logged in as:</strong> {loggedInUser.name}
                    {loggedInUser.isAdmin && <span className="badge bg-danger ms-2">Admin</span>}
                </div>
            </>
        ) : (
      ...
      {loggedInUser && (
        <div className="review-card mb-4">
            ...
            <StarRating
                rating={newReview.rating} editable={true} hoverRating={newReview.hoverRating}
                onRatingChange={(r) => setNewReview({...newReview, rating: r})}
                onHoverChange={(r) => setNewReview({...newReview, hoverRating: r})}
                showNumber={false}
            />
            </div>
            <div className="mb-3">
            ...
            <div className="d-flex gap-2">
                ...
                />
                <textarea
                    className="form-control" id="comment" rows="3" placeholder="Write your review here..."
            ...
            {isPosting ? (
                ...
                <><i className="bi bi-send me-2"></i> Post Review</>
            )}
            </button>
        </div>
      )}

      {/* Loading State for List only */}
      {loading ? (
        <div className="text-center p-5"><div className="spinner-border text-primary"></div></div>
      ) : reviews.length === 0 ? (
        <div className="text-center p-5 border rounded bg-light">
            <i className="bi bi-chat-square-quote display-4 text-muted mb-3"></i>
        ...
        visibleReviews.map((review) => {
            ...
            <div key={review.id} className={`review-card ${review.flagged ? 'flagged' : ''}`}>
                ...
                <div className="d-flex align-items-center">
                    <img 
                        src={`${review.photo || DEFAULT_PROFILE_PIC}?t=${new Date(review.time).getTime()}`}//{review.photo || DEFAULT_PROFILE_PIC} 
                        alt={review.name} 
                        className="user-avatar"
                        onError={(e) => e.target.src = DEFAULT_PROFILE_PIC} 
                    />
                    ...
                    {review.name}
                    {loggedInUser && review.userId === loggedInUser.id && <span className="badge bg-primary ms-2">You</span>}
                    </span>
                ...
                {/* --- TOGGLE VIEW: EDITING VS VIEWING --- */}
                {isEditing ? (
                  // EDIT MODE FORM
                  <div className="edit-form mt-2 mb-3 p-3 bg-light rounded">
                    ...
                    <div className="d-flex gap-2">
                        <button className="btn btn-sm btn-success" onClick={() => handleSaveEdit(review.id)} disabled={isSaving}>
                          {isSaving ? 'Saving...' : 'Save Changes'}
                        </button>
                        <button className="btn btn-sm btn-outline-secondary" onClick={handleCancelEdit} disabled={isSaving}>
                          Cancel
                        </button>
                    </div>
                  </div>
                  ...
                  // STANDARD VIEW MODE
                  <>
                    <div className="mb-2"><StarRating rating={review.rating} showNumber={true} /></div>
                    <p className="mb-3 comment-text">{review.comment}</p>
                  </>
                )}

                {/* Action Buttons (Like | Dislike | Reply | Edit | Delete) */}
                {!isEditing && (
                  <div className="action-buttons">
                    <button className={`action-btn like-btn ${review.userLiked ? 'active' : ''}`} onClick={() => handleLike(review.id)}>
                        <i className={`bi ${review.userLiked ? 'bi-hand-thumbs-up-fill' : 'bi-hand-thumbs-up'}`}></i>
                        <span>{review.likes}</span>
                    </button>
                    <button className={`action-btn dislike-btn ${review.userDisliked ? 'active' : ''}`} onClick={() => handleDislike(review.id)}>
                        <i className={`bi ${review.userDisliked ? 'bi-hand-thumbs-down-fill' : 'bi-hand-thumbs-down'}`}></i>
                        <span>{review.dislikes}</span>
                    </button>
                    
                    {loggedInUser && (
                        <button className="action-btn" onClick={() => setReplyInput({...replyInput, [review.id]: replyInput[review.id] || ""})}>
                            <i className="bi bi-reply"></i><span>Reply</span>
                        </button>
                    )}

                    {/* EDIT BUTTON */}
                    {canEdit(review.userId) && (
                          <button className="action-btn" onClick={() => handleStartEdit(review)} title="Edit review">
                    ...
                    <button className="action-btn" onClick={() => handleDelete(review.id)} title="Delete review">
                ...
                <div className="reply-input-container">
                    <div className="d-flex gap-2 align-items-center">
                        <img 
                             src={loggedInUser?.photo || DEFAULT_PROFILE_PIC} 
                        ...
                        <div className="input-group">
                            <input
                                type="text" className="form-control" placeholder={`Reply as ${loggedInUser.name}...`}
                                value={replyInput[review.id]} onChange={(e) => setReplyInput({...replyInput, [review.id]: e.target.value})}
                                onKeyPress={(e) => e.key === 'Enter' && handlePostReply(review.id)}
                            />
                            <button className="btn btn-outline-primary" onClick={() => handlePostReply(review.id)}><i className="bi bi-send"></i></button>
                        </div>
                    </div>
                </div>
                )}
                ...
                <div className="replies-section">
                    ...
                    {review.showReplies ? (
                        <><i className="bi bi-chevron-up me-1"></i> Hide {review.replies.length} replies</>
                    ) : (
                        <><i className="bi bi-chevron-down me-1"></i> Show {review.replies.length} replies</>
                    )}
                    </button>
                    ...
                    return (
                        ...
                        <div className="d-flex justify-content-between align-items-center mb-2">
                            <div className="d-flex align-items-center">
                            <img 
                                src={reply.photo || DEFAULT_PROFILE_PIC} 
                                alt={reply.name} 
                                className="user-avatar small" 
                                onError={(e) => e.target.src = DEFAULT_PROFILE_PIC}
                            />
                            <span className="user-badge">
                                {reply.name}
                                {loggedInUser && reply.userId === loggedInUser.id && <span className="badge bg-primary ms-2">You</span>}
                            </span>
                            ...
                            {canDelete(reply.userId) && (
                                <button className="action-btn" onClick={() => handleDelete(review.id, reply.id)} title="Delete reply">
                                <i className="bi bi-trash"></i>
                                </button>
                            )}
                            </div>
                        ...
                        <div className="action-buttons">
                            <button className={`action-btn like-btn ${reply.userLiked ? 'active' : ''}`} onClick={() => handleLike(review.id, reply.id)}>
                            <i className={`bi ${reply.userLiked ? 'bi-hand-thumbs-up-fill' : 'bi-hand-thumbs-up'}`}></i><span>{reply.likes}</span>
                            </button>
                            <button className={`action-btn dislike-btn ${reply.userDisliked ? 'active' : ''}`} onClick={() => handleDislike(review.id, reply.id)}>
                            <i className={`bi ${reply.userDisliked ? 'bi-hand-thumbs-down-fill' : 'bi-hand-thumbs-down'}`}></i><span>{reply.dislikes}</span>
                            </button>
                        </div>
                        </div>
                    );
